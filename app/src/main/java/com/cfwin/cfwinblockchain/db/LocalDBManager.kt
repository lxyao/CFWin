...
class LocalDBManager constructor(context: Context) : AbsDBManager(context, Constant.DB_NAME, Constant.DB_VERSION) {
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        val userSql = "create table if not exists user(_id INTEGER PRIMARY KEY AUTOINCREMENT,userName varchar(20),accountName varchar(20),address text not null,type int, integral varchar(20))"
        db?.execSQL(userSql)
